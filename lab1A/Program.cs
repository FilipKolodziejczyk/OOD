        ...
        static void Main(string[] args)
        {
			...
			//Part 2
			{
                for (int i = 0; i < nodesDatabase.Length; i++)
                    nodesDatabase[i] = nm.SetNodeToBroken(nm.DecryptMessages(nodesDatabase[i], 2), "testing");

                for (int i = 0; i < nodesDatabase.Length; i++)
                    nodesDatabase[i] = nm.MakeNodeSmart(nodesDatabase[i]);

                foreach (var node in nodesDatabase)
                    if (node.GetName() == "Third node" && node.GetCapacity() == 10)
                        TestPrintNode(node,1, "test");
